define('dartpad_main', ['dart_sdk', 'flutter_web'], (function load__dartpad_main(dart_sdk, flutter_web) {
    ...
    var SunflowerPainterL = () => (SunflowerPainterL = dart.constFn(dart.legacy(main.SunflowerPainter)))();
    ...
    var doubleLToNullN = () => (doubleLToNullN = dart.constFn(dart.fnType(core.Null, [doubleL()])))();
    const CT = Object.create(null);
    var L0 = "file:///tmp/dartpadRZUWWU/main.dart";
    ...
    main.SunflowerPainter = class SunflowerPainter extends custom_paint.CustomPainter {
        ...
        paint(canvas, size) {
            ...
            for (let i = 0; i < dart.notNull(this.seeds); i = i + 1) {
                let theta = i * 6.283185307179586 / dart.notNull(main.SunflowerPainter.phi);
                let r = math.sqrt(i) * 4;
        ...
        drawSeed(canvas, x, y) {
            ...
            canvas.drawCircle(new ui.Offset.new(dart.nullCast(x, core.double), dart.nullCast(y, core.double)), 2, paint);
        }
    };
    ...
    dart.setMethodSignature(main.SunflowerPainter, () => ({
        ...
        drawSeed: dart.fnType(dart.void, [dart.legacy(ui.Canvas), dart.legacy(core.num), dart.legacy(core.num)])
    }));
    ...
    dart.defineLazy(main.SunflowerPainter, {
    ...
    main._SunflowerState = class _SunflowerState extends framework.State$(dart.legacy(main.Sunflower)) {
        get seedCount() {
            return this.seeds[$floor]();
        }
        build(context) {
            return new app.MaterialApp.new({
                debugShowCheckedModeBanner: false, theme: theme_data.ThemeData.new().copyWith({ platform: main.platform, brightness: ui.Brightness.dark, sliderTheme: slider_theme.SliderThemeData.fromPrimaryColors({ primaryColor: main.primaryColor, primaryColorLight: main.primaryColor, primaryColorDark: main.primaryColor, valueIndicatorTextStyle: new text.DefaultTextStyle.fallback().style }) }), home: new scaffold.Scaffold.new({
                    appBar: new app_bar.AppBar.new({ title: new text.Text.new("Sunflower") }), drawer: new drawer.Drawer.new({ child: new scroll_view.ListView.new({ children: JSArrayOfWidgetL().of([new drawer_header.DrawerHeader.new({ child: new basic.Center.new({ child: new container.Container.new({ child: new text.Text.new("Sunflower 🌻", { style: new text_style.TextStyle.new({ fontSize: 32.0 }) }) }) }) })]) }) }), body: new container.Container.new({
                        constraints: new box.BoxConstraints.expand(), decoration: new box_decoration.BoxDecoration.new({ border: box_border.Border.all({ color: colors.Colors.transparent }) }), child: new basic.Column.new({
                            crossAxisAlignment: flex.CrossAxisAlignment.center, mainAxisAlignment: flex.MainAxisAlignment.start, children: JSArrayOfWidgetL().of([new container.Container.new({ decoration: new box_decoration.BoxDecoration.new({ border: box_border.Border.all({ color: colors.Colors.transparent }) }), child: new basic.SizedBox.new({ width: 400.0, height: 400.0, child: new basic.CustomPaint.new({ painter: new main.SunflowerPainter.new(this.seedCount) }) }) }), new text.Text.new("Showing " + dart.str(this.seedCount) + " seeds"), new basic.ConstrainedBox.new({
                                constraints: new box.BoxConstraints.tightFor({ width: 300.0 }), child: new slider.Slider.adaptive({
                                    min: 20.0, max: 2000.0, value: this.seeds, onChanged: dart.fn(newValue => {
    ...
    dart.trackLibraries("dartpad_main", {
        "file:///tmp/dartpadRZUWWU/bootstrap.dart": bootstrap,
        "file:///tmp/dartpadRZUWWU/main.dart": main
    }, {
    }, null);
    // Exports:
    return {
        tmp__dartpadRZUWWU__bootstrap: bootstrap,
        tmp__dartpadRZUWWU__main: main
    };
}));
